...
def hukagawa_news(client)
    ...
    i = 1
    while i < 3
        if news.search("div.content_wrap ul li[#{i}] div.date").inner_text == "#{DateTime.now.year}.#{DateTime.now.month}.#{DateTime.now.day - 1}" || news.search("div.content_wrap ul li[#{i}] div.date").inner_text == "#{DateTime.now.year}.#{DateTime.now.month}.0#{DateTime.now.day-1}" || news.search("div.content_wrap ul li[#{i}] div.date").inner_text == "#{DateTime.now.year}.0#{DateTime.now.month}.0#{DateTime.now.day-1}"
...
def view_memo(client,session)
    ...
    (3..sheet.num_rows).each do |row|
        if sheet[row,2].empty? == false && sheet[row,2] != ""
            client.update("[メモ#{row-2}]\n#{sheet[row,2]}\n\n-Reminder at #{DateTime.now.hour}:#{DateTime.now.minute}-")
        end
    end
end
...
def trans_from_En_to_Ja(client,text,tweet)
    #認証情報
    translate_key= ENV['MY_TRANSLATE_KEY']
    translate_secret= ENV['MY_TRANSLATE_SECRET']
    name = ENV['MY_TRANSLATE_NAME']
...
begin
    ...
    #履修科目
    subjects = ["学習工学特論","セキュリティ基礎","データマイニング"]
